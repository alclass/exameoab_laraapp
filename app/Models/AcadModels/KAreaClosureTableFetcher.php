...
class KAreaClosureTableFetcher {
  ...
  */
  ...
  public static function fetch_level_knowledge_areas_as_ids($level_n=1) {
    if ($level_n < 2) {
      return KnowledgeArea::get_root_knowledge_area();
    }
    ...
    return $level_ka_ids;
  } // ends fetch_level_knowledge_areas_as_ids()

  public static function fetch_level_knowledge_areas_as_objs($level_n=1) {
    $level_knowledge_areas_as_ids = self::fetch_level_knowledge_areas_as_ids($level_n);
    $level_knowledge_areas_as_objs = KnowledgeArea::find($level_knowledge_areas_as_ids);
    return $level_knowledge_areas_as_objs;
  } // ends fetch_level_knowledge_areas_as_objs()

  public static function fetch_subtree_knowledge_areas_as_ids($node_id) {
    /*
  ...
  } // ends fetch_subtree_knowledge_areas_as_ids()

  public static function fetch_subtree_knowledge_areas_as_objs($node_id) {
    /*
      This method wraps around the former one [fetch_subtree_knowledge_areas_as_ids()].
      Its purpose is to retrieve all knowledge areas belonging to a certain subtree (or branch).
      The former fetch_subtree_knowledge_areas_as_ids() fetches the id's,
    ...
    */
    $subtree_knowledge_areas_as_ids = self::fetch_subtree_knowledge_areas_as_ids($node_id);
    $path_from_root_to_karea_as_objs = KnowledgeArea::find($subtree_knowledge_areas_as_ids);
    return $path_from_root_to_karea_as_objs;
  } // ends fetch_subtree_knowledge_areas_as_objs()
